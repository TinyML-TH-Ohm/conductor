...
// tiny-ml-25-0xxx-tinyml
#define BLE_SENSE_UUID(val) ("74696e79-6d6c-3235-0" val "-74696e796d6c")
...
BLEService service(BLE_SENSE_UUID("000"));
BLECharacteristic strokeCharacteristic(BLE_SENSE_UUID("001"), BLERead,
                                       stroke_struct_byte_count);
BLECharacteristic predictionCharacteristic(BLE_SENSE_UUID("002"), BLENotify, 2);
...
void EstimateGravityDirection(float *gravity) {
  ...
  const int start_index =
    ((acceleration_data_index + (acceleration_data_length - (3 * (samples_to_average + 1)))) % acceleration_data_length);
...
void UpdateVelocity(int new_samples, float *gravity) {
  ...
  const int start_index =
    ((acceleration_data_index + (acceleration_data_length - (3 * (new_samples + 1)))) % acceleration_data_length);
...
void EstimateGyroscopeDrift(float *drift) {
  ...
  const int start_index =
    ((gyroscope_data_index + (gyroscope_data_length - (3 * (samples_to_average + 1)))) % gyroscope_data_length);
...
void UpdateOrientation(int new_samples, float *gravity, float *drift) {
  ...
  const int start_index =
    ((gyroscope_data_index + (gyroscope_data_length - (3 * new_samples))) % gyroscope_data_length);
...
bool IsMoving(int samples_before) {
  ...
  const int start_index =
    ((gyroscope_data_index + (gyroscope_data_length - (3 * (moving_sample_count + samples_before)))) % gyroscope_data_length);
...
void setup() {
  ...
  service.addCharacteristic(predictionCharacteristic);
